...
import bannerImage from '../assets/ttgt.jpg'; 
...
export const MainPage = () => {
  ...
  const iconMap: Record<string, React.ReactNode> = {
    Cpu: <Cpu size={24} />, Zap: <Zap size={24} />, ShieldCheck: <ShieldCheck size={24} />,
    BookOpen: <BookOpen size={24} />, Layout: <Layout size={24} />, Code: <Code size={24} />,
    Atom: <Atom size={24} />, Calculator: <Calculator size={24} />, FlaskConical: <FlaskConical size={24} />,
    Globe: <Globe size={24} />, HardDrive: <HardDrive size={24} />, Terminal: <Terminal size={24} />,
    Settings: <Settings size={24} />, Database: <Database size={24} />, Activity: <Activity size={24} />
  };
  ...
  if (loading) return <div className="flex items-center justify-center min-h-[400px] font-black italic text-[#1976d2] animate-pulse uppercase tracking-widest">АНАЛИЗ УЧЕБНОЙ ПРОГРАММЫ...</div>;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 space-y-10 animate-in fade-in duration-700 font-black italic uppercase text-slate-700">
      ...
      <div className="relative w-full min-h-[320px] sm:h-[400px] rounded-[3rem] overflow-hidden shadow-2xl flex items-center border-4 border-white">
        ...
        <img src={bannerImage} className="absolute inset-0 w-full h-full object-cover" alt="ТТЖТ Баннер" />
        
        <div className="absolute inset-0 z-20 p-6 sm:p-14 flex flex-col justify-center max-w-2xl space-y-4 text-white">
          <h2 className="text-3xl sm:text-6xl tracking-tighter leading-none italic font-black">ОБРАЗОВАТЕЛЬНАЯ ПЛАТФОРМА ТТЖТ</h2>
        ...
        <div className="absolute right-12 bottom-12 z-20 text-white text-right hidden sm:block">
           <div className="text-9xl opacity-50 leading-none font-black">{now.getDate()}</div>
           <div className="text-xl tracking-tighter mt-[-10px] font-bold">{dateStr}</div>
        </div>
      </div>
      ...
      <div className="space-y-8 pb-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {subjects.map((s) => (
            <div key={s.id} onClick={() => navigate(`/subject/${s.id}`)} className="relative group cursor-pointer bg-white rounded-[2.5rem] shadow-xl hover:shadow-blue-200 transition-all overflow-hidden h-64 border border-blue-50 hover:-translate-y-2">
              ...
              <div className="absolute inset-0 z-20 p-8 flex flex-col justify-between text-white">
                <div className="bg-white/20 p-4 rounded-2xl w-fit backdrop-blur-md group-hover:scale-110 transition-transform">{iconMap[s.iconName] || <Layout size={24} />}</div>
                <div>
                  <h4 className="text-xl leading-tight mb-2 break-words font-black italic uppercase">{s.title}</h4>
                  <div className="flex justify-between items-center">
                    <p className="text-[10px] opacity-70 tracking-widest font-black italic">{s.sections.length} РАЗДЕЛОВ</p>
